...
function runTest({ dev }: { dev: boolean }) {
  describe(`Basic Features  [${dev ? 'dev' : 'prod'}]`, () => {
    let ctx: AppCtx;
    let page: Page;
    ...
    describe('Head', () => {
      ...
      test('should overwrite the default head tags', async () => {
        ...
        expect(
          await localPage.$attr('meta[name="viewport"]', 'content')
        ).toEqual('width=device-width');
      });
...
describe('HTML document', () => {
  ...
  test('should have hash in script resource url', async () => {
    page = await ctx.browser.page(ctx.url('/'));
    const scripts = await page.$$attr('script', 'src');
    scripts.forEach(s => {
      if (s) {
        expect(s).toMatch(/.[a-f0-9]{8,8}.js/);
      }
    });
  });
});
