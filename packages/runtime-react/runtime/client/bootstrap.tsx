/// <reference path="../../client-env.d.ts" />

import React from "react";
import ReactDom from "react-dom";
import * as Runtime from "@shuvi/types/runtime";
import { Router } from "react-router-dom";
import App from "../shared/app";
import Loadable from "@shuvi/runtime-react/lib/loadable";
import { createBrowserHistory } from "@shuvi/runtime-react/lib/router/history";
// @ts-ignore
import { setHistory } from "@shuvi/runtime-react/lib/router/router";

export const bootstrap: Runtime.Bootstrap = async ({
  appData,
  appContainer
}) => {
  await Loadable.preloadReady(appData.dynamicIds);

  // TODO: hash history(tree shaking)
  // TODO: config basename
  const history = createBrowserHistory({ basename: "/" });
  setHistory(history);

  return ReactDom.render(
    <Router history={history}>
      <App />
    </Router>,
    appContainer
  );
};
